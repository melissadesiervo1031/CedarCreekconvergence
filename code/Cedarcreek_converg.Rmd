...
PcOAdecade<-ggplot(data=PcoAplayingfirst2, aes(x=meanAxis1  , y=meanAxis2 , color=ntrt2)) +
  ...
  geom_segment(data=wide_PcoAplaying1, aes(x=`1982`  , y=`1982`, xend =`1992`, yend = `1992`, color=ntrt2), arrow=arrow(length=unit(0.0, "cm"))) +
    ...
    scale_shape_manual(values = c(21, 22, 24))+
      theme(strip.background = element_blank())+
      geom_text(aes(x = -0.27, y = 0.15, label = abclabel, group =     abclabel),size = 4, color= "black", check_overlap = T)+theme(strip.text = element_blank())
...
head(exp12subset_2)
dim(exp12subset_2)  ##data frame with everything, minus the years where not everything was sampeled## biomass data are log transformed####
...
sinuosityplot<- ggplot(data=sinuosityavg2, aes(x=ntrt2, y=sinuosityavg, group=ntrt2, color=ntrt2)) +
  ...
  theme(strip.text.x = element_text(size = 14, colour = "black"))+
   ...
   theme(strip.text = element_text(size=12, face="bold"))+
     theme(strip.background = element_blank())+
      geom_text(aes(x = 0.8, y = 8.2, label = abclabel, group =     abclabel),size = 4, color= "black", check_overlap = T)+ scale_y_continuous(labels = scales::number_format(accuracy = 0.1))
...
trajdistnonlin2 <- nls(meantrajdist ~ SSlogis(year2, Asym, xmid, scal), data=avgTraj_byyear_E001E002_2) ## this works but is a diff model than we've been using up till this point...###
...
rawaic<-AIC(trajdistnonlin3,trajdistlin, trajdistnull)
nR<-dim(avgTraj_byyear_E001E002_2)[1]  #Sample size 
aictable(rawaic,nR)

##model with a slope performs better than a model w/ slope, and better than nonlinear model###
...
avgTraj_byyear<- ggplot(data=avgTraj_byyear_E001E002_2) +
    geom_point(data=avgTraj_byyear_E001E002_2, aes(x=as.numeric(as.character(year2)), y=meantrajdist, group=ntrt2, color=ntrt2)) +
    geom_point(data=avgTrajcontrol, aes(x=as.numeric(as.character(year2)), y=meantrajdist),     color="gray45") +
  geom_smooth(data=avgTraj_byyear_E001E002_2, aes(x=as.numeric(as.character(year2)), y=meantrajdist, group=ntrt2, color=ntrt2), method="loess", se=FALSE)+
    facet_grid(~exp)+ 
    ylab(expression('Annual community trajectory distance'))+
    xlab("Time since experiment (years)")+
    mytheme+
  ...
  theme(legend.position = "none") +
    ...
    theme(strip.text = element_text(size=12, face="bold"))+
     theme(strip.background = element_blank())+
 geom_text(aes(x = 2.0, y = 0.25, label = abclabel, group =     abclabel),size = 4, color= "black", check_overlap = T)+ylim(0, 0.27)
...
```{r - Indicator species analyses - 3 time periods log data}

##start w/ exp12subset_2#

head(exp12subset_2) ##log transformed values###


###subset to the controls and high N treatments###

exp12subset_4<-subset(exp12subset_2,ntrt==9|ntrt==6)
...
vegwideplotdata3$stagentrtdist2 <- factor(vegwideplotdata3$stagentrtdist2, levels = c("1", "2", "3", "4", "5", "6", "7", "8"))
