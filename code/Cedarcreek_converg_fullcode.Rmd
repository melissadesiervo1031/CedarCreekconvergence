...
sum(plotcountcheck19822004$n) #  6102 total plots 1982 to 2004 #### matches exactly w/ excel file## :-) 
...
###CODE CHECKING GOT UP TO HERE on 10/27#######


###PCOA across years and all fields including field D ###

```{r - Conduct a PCoA fields ABCD (log)}
...
fieldsABCDall<-dplyr::bind_rows(d3woody, da_fieldD_3)


##subset to years before 2004###

fieldsABCD1982_2004<-subset(fieldsABCDall, year<2005)


# Transpose data to be a site by species matrix
fieldsABCD_wide <- reshape(fieldsABCD1982_2004,
                   v.names="mass.above",
                   idvar=c("field", "exp", "plot", "year"),
                   timevar="species",
                   direction="wide") 

# Fill in NA's with zeros in wide data set
fieldsABCD_wide[is.na(fieldsABCD_wide)] <- 0


##arrange the dataframe so that that columns and rows are in right order##
fieldsABCD_wide_sorted<-fieldsABCD_wide%>%arrange(year)%>%arrange(plot)%>%arrange(field)%>%arrange(exp)


##just the plot data##
plotABCD<-fieldsABCD_wide_sorted%>% dplyr::select('field':'ntrt2')

##just the veg data##

vegABCD<-fieldsABCD_wide_sorted%>% dplyr::select(`mass.above.ACER NEGUNDO`:`mass.above.SOLANUM DULCAMARA`)  ###double check selecting all veg columns##
...
### PCOA ABCD both E001 and E002####

PCOA_ABCD <- pcoa(distABCD)  ## takes a while##

## extracting the axis scores for each exp##

PCOAaxesABCD <- PCOA_ABCD$vectors[,c(1,2)]


##merge the axis scores with the plot data ###

PCOAandplotsABCD<-cbind(plotABCD,PCOAaxesABCD  )

##PCOAandplotsABCD has the plot info and the PCOA scores for the entire experiment (E001 and E002, all three fields combined, with a subsetted selection of the ntrt treatments). ALSO INCLUDES FIELD D. Only goes up to 2004. ###
...
plotABCD2<-plotABCD%>% mutate(fieldexpplot= paste(field, exp,plot, sep = '_'))

##
plotABCD3<-distinct(plotABCD2, fieldexpplot, .keep_all=TRUE)


trajdirection<-trajectoryDirectionality(distABCD, plotABCD2$fieldexpplot, plotABCD2$year)


trajdirectdf<-cbind(plotABCD3, trajdirection)


##mean directionality for each exp_ntrt including field d ###
...
##
```{r - calculate trajectory distance between years BC dist matrix (log data)}
...
###split it by E001 + field D and E002 since there are missing years in E002##
BCdistE001andD<-subset(BCdistABCDdf2, exp==1|exp==0) 
BCdistE002<-subset(BCdistABCDdf2, exp==2) 

BCmatrixE001andD<-as.matrix(BCdistE001andD[,11:4761])
BCmatrixE002<-as.matrix(BCdistE002[,11:4761])

plotABCDE001<-subset(plotABCD2, exp==1|exp==0)
plotABCE002<-subset(plotABCD2, exp==2)


###

####
trajlengthall<-trajectoryLengths(BCdistE001andD,  plotABCDE001$fieldexpplot,  plotABCDE001$year)
