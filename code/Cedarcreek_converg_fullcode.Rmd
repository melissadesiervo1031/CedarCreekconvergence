...
### figure for presentation 12/5/2022####
...
```{r - make figure pcoa for research}
...
exp12subsetsorted<-exp12subset_1%>%arrange(year)%>%arrange(plot)%>%arrange(field)%>%arrange(exp)
...
##PCOAandplotsboth N has the plot info and the PCOA scores for the entire experiment (E001 and E002, all three fields combined, with a subset of the ntrt treatments)


###calculate averages of plots (PCOA and plots both)
...
### subset to the end of time series###

PCOAandplotsboth$year<-as.numeric(as.character(PCOAandplotsboth$year))

PCOAandplots2004<-subset(PCOAandplotsboth, year==2004)

PCOAandplotsavg2004<-subset(PCOAandplotsavg, year==2004)



PCOAandplots2004$ntrt2=PCOAandplots2004$ntrt
PCOAandplotsavg2004$ntrt2=PCOAandplotsavg2004$ntrt
...
PCOAandplotsavg2004$ntrt2  <- mapvalues(PCOAandplotsavg2004$ntrt2, from=c("9", "1", "2", "4", "6"),
                               to=c("None", "0 N + μ", "1 N + μ", "3.4 N + μ","9 N + μ"))

PCOAandplots2004$ntrt2  <- factor(PCOAandplots2004$ntrt2 , levels = c("None", "0 N + μ", "1 N + μ", "3.4 N + μ","9 N + μ"))

PCOAandplotsavg2004$ntrt2  <- factor(PCOAandplotsavg2004$ntrt2 , levels = c("None", "0 N + μ", "1 N + μ", "3.4 N + μ","9 N + μ"))


PCOAandplots2004$disk<-plyr::mapvalues(PCOAandplots2004$disk, from=c("0", "1"), to=c("Intact in 1982", "Disturbed in 1982"))

PCOAandplotsavg2004$disk<-plyr::mapvalues(PCOAandplotsavg2004$disk, from=c("0", "1"), to=c("Intact in 1982", "Disturbed in 1982"))
...
PcOA2004<-ggplot(data=PCOAandplotsavg2004, aes(x=meanAxis1  ,y=meanAxis2 , color=ntrt2)) +
  geom_hline(yintercept = 0)+geom_vline(xintercept = 0)+
  geom_point(data=PCOAandplots2004, aes(x=Axis.1, y= Axis.2, color= ntrt2), size=4, alpha=0.2) +
  ...
  geom_point(size=6)+
    ...
    scale_shape_manual(values = c(21, 22, 24))+
      theme(strip.background = element_blank())+theme(strip.text.x = element_text(size = 14))

    

pdf("C:/Users/Melissa/Dropbox/DRAGnet/CedarCreekdata/PCOA2004.pdf", width = 8, height = 4)
PcOA2004
dev.off()
...
pdf("C:/Users/Melissa/Dropbox/DRAGnet/CedarCreekdata/Dist recovery MS/Figures_overleaf/variancetimeall_new2.pdf", width = 7, height = 3)
variancetimeall_allnut
dev.off()
...
mytheme<- theme_bw()+ theme(axis.line.x= element_line(colour = "black", size=0.3))+theme(axis.line.y= element_line(colour = "black", size=0.3))+theme(axis.text.x=element_text(size=7, colour = "black"))+theme(axis.text.y=element_text(size=7, colour = "black"))+theme(axis.title=element_text(size=10))+theme(plot.title=element_text(size=7) +theme(plot.title = element_text(hjust = 0.5)))+ theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+ theme(legend.title = element_blank())+theme(plot.title = element_text(margin=margin(0,0,5,0)))+ theme(panel.grid.major = element_line(colour = "gray 85", size=0.25))+theme(legend.text = element_text(size=8))+theme(legend.key.size = unit(0.45, 'cm'))
...
pdf("C:/Users/Melissa/Dropbox/DRAGnet/CedarCreekdata/Dist recovery MS/Figures_overleaf/PCOA12panel.pdf", width = 9, height = 4.5)
PcoAfieldAveragedE001E002_fullN
dev.off()
...
PCOAandplots198219922004_fullN$ntrt2 <- factor(PCOAandplots198219922004_fullN$ntrt2, levels = c("None", "0 N+μ", "1 N + μ", "2 N + μ", "3.4 N + μ","5.4 N + μ","9 N + μ", "17 N + μ", "27.2 N + μ"))
...
PcoAplayingfirst2_fullN$ntrt2 <- factor(PcoAplayingfirst2_fullN$ntrt2, levels = c("None", "0 N+μ", "1 N + μ", "2 N + μ", "3.4 N + μ","5.4 N + μ","9 N + μ", "17 N + μ", "27.2 N + μ"))
...
pdf("C:/Users/Melissa/Dropbox/DRAGnet/CedarCreekdata/Dist recovery MS/Figures_overleaf/speciesPCOA.pdf", width = 7, height = 5)
speciesPCOAplot
dev.off()
...
##########GOT UP TO HERE ON 11/22/22 ##### STILL NEED TO UPLOAD NEW SINUOSITY, SPEED, AND OTHER SUPPLEMENTAL FIGURES
...
sinuosityavg2$ntrt2 <- factor(sinuosityavg2$ntrt2, levels = c("None", "0 N+μ", "1 N + μ", "2 N + μ", "3.4 N + μ","5.4 N + μ","9 N + μ", "17 N + μ", "27.2 N + μ"))
...
sinuosityavgbydecade2$ntrt2 <- factor(sinuosityavgbydecade2$ntrt2, levels = c("None", "0 N+μ", "1 N + μ", "2 N + μ", "3.4 N + μ","5.4 N + μ","9 N + μ", "17 N + μ", "27.2 N + μ"))
...
trajdiravg2$ntrt2  <- mapvalues(trajdiravg2$ntrt2, from=c("0", "9", "1", "2", "3", "4","5","6", "7", "8"),
                               to=c("Never", "None", "0 N+μ", "1 N + μ", "2 N + μ", "3.4 N + μ","5.4 N + μ","9 N + μ", "17 N + μ", "27.2 N + μ"))

trajdiravg2$ntrt2 <- factor(trajdiravg2$ntrt2, levels = c("Never", "None", "0 N+μ", "1 N + μ", "2 N + μ", "3.4 N + μ","5.4 N + μ","9 N + μ", "17 N + μ", "27.2 N + μ"))
...
avgTraj_byyear_E001E002_2$ntrt2 <- factor(avgTraj_byyear_E001E002_2$ntrt2, levels = c("Never", "None", "0 N+μ", "1 N + μ", "2 N + μ", "3.4 N + μ","5.4 N + μ","9 N + μ", "17 N + μ", "27.2 N + μ"))
...
richnessaverage_3$ntrt2  <- mapvalues(richnessaverage_3$ntrt2, from=c("9", "1", "2", "4","6"), to=c("None", "0 N+μ", "1 N + μ", "3.4 N + μ","9 N + μ"))

richnessaverage_3$ntrt2 <- factor(richnessaverage_3$ntrt2, levels = c("None", "0 N+μ", "1 N + μ", "3.4 N + μ","9 N + μ"))
...
totalbiomassdf22$ntrt2  <- mapvalues(totalbiomassdf22$ntrt2, from=c("9", "1", "2", "4","6"), to=c("None", "0 N+μ", "1 N + μ", "3.4 N + μ","9 N + μ"))

totalbiomassdf22$ntrt2 <- factor(totalbiomassdf22$ntrt2, levels = c("None", "0 N+μ", "1 N + μ", "3.4 N + μ","9 N + μ"))
...
sumfuntionalgroupdf2$ntrt2  <- mapvalues(sumfuntionalgroupdf2$ntrt2, from=c("9", "1", "2", "4","6"), to=c("None", "0 N+μ", "1 N + μ", "3.4 N + μ","9 N + μ"))

sumfuntionalgroupdf2$ntrt2 <- factor(sumfuntionalgroupdf2$ntrt2, levels = c("None", "0 N+μ", "1 N + μ", "3.4 N + μ","9 N + μ"))
