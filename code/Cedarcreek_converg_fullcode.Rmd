...
####Commenting this out for now....might put it back in 10/10/22###
...
da["mass.above"][da["mass.above"] == 1711.970] <- 1711.970/10
...
###dataframe da still includes nutrient cessation plots get rid of those.......####

da2<-subset(da, ntrt.origin==1)

### Dataframe da2 still may have some plots that got burned? Need to ask Eric about this...10/10/2022
...
d2woody <- da.mn[da.mn$sorted ==1 & da.mn$live ==1, c("field", "exp","plot", "subplot", "year", "disk", "ntrt", "nadd", "species", "mass.above")]
...
dim(da.wide_allwhole) ## dimensions of the dataset = 8798 X 234##
...
##subset to years before 2005 For E001, E002 goes to 2019 ## (bc of change in fire regime)

exp12004exp2full<-subset(da.wide5, exp==2 | exp==1 & year<2005)
...
##BEST THING WOULD BE TO START W/ DATA SET FOR E001 that only goes to 2004, and E002 to 2019##

exp12004exp2full<-subset(da.wide5, exp==2 | exp==1 & year<2005)

##just the veg data##

veg_full<-exp12004exp2full%>% dplyr::select(`mass.above.ACHILLEA MILLEFOLIUM(LANULOSA)`:`mass.above.VIOLA SP.`)
...
bd_allbyfield<-betadisper(vegdist(logvegfull),exp12004exp2full$expntrtfieldyear, type = "centroid")    ####Note that you have to specify the centroid ##  ##takes a LONG TIME ##
...
bd_allbyfield_2_df<-bd_allbyfield_2_df%>%mutate(ntrt2 = ntrt)
...
disttocentroidplotBYFIELD<- ggplot(bd_allbyfield_2_df) +
    geom_point(data=bd_allbyfield_2_df, aes(x=as.numeric(as.character(year)), y=distances, group=ntrt2, color=ntrt2)) +
   geom_smooth(data=bd_allbyfield_2_df,   aes(x=as.numeric(as.character(year)), y=distances, group=ntrt2, color=ntrt2), method="loess", se=FALSE) +
